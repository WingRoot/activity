...
class TestGame < Test::Unit::TestCase
        ...
        def test_add_and_get_kills
                ...
                assert_equal(1,g.player("joao").kills)
                g.subtract_kills("joao")
                assert_equal(0, g.player("joao").kills)
                g.update_kills_by_means("joao", "shotgun")
                assert_equal({"shotgun"=> 1},g.player("joao").kills_by_means)
                g.update_deaths_by_means("joao", "queda de arvore")
                assert_equal({"queda de arvore"=>1}, g.player("joao").deaths_by_means)

                assert_raise(ParserQuakeException::MustBeString){g.update_kills_by_means("joao",1)}
                assert_raise(ParserQuakeException::MustBeString){g.update_deaths_by_means("joao",1)}
                assert_nothing_raised{g.subtract_kills("joao")}
                assert_nothing_raised{g.add_kills("joao")}
